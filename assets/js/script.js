...
function renderPortfolio(data) {
  ...
  document.getElementById('app').innerHTML = `
    ...
    <section id="resume" class="section resume-section"><div class="container" data-anim><div class="section-heading"><div><p class="eyebrow">06 / RESUME</p><h2>把经历整理成<br><em>清晰的路径。</em></h2></div><button class="btn primary" type="button" data-resume-download>下载简历 ↓</button></div><div class="resume-meta"><div><span class="resume-label">教育背景</span><strong>${escapeHtml(data.resume.education)}</strong></div><div><span class="resume-label">关注方向</span><strong>${data.resume.keywords.map(escapeHtml).join(' · ')}</strong></div></div></div></section>

    <section id="contact" class="section contact-section"><div class="container contact-layout" data-anim><div><p class="eyebrow">07 / CONTACT</p><h2>有个想法？<br><em>一起聊聊。</em></h2><p class="section-copy">${escapeHtml(profile.role)}，期待听到你的项目、问题或一个还在发芽的念头。</p><div class="contact-links"><a href="mailto:${escapeHtml(profile.email)}">${escapeHtml(profile.email)}</a><a href="${escapeHtml(profile.github)}" target="_blank" rel="noopener">GitHub ↗</a></div></div><form id="contact-form" class="contact-form"><label><span>你的名字</span><input type="text" id="cf-name" placeholder="怎么称呼？" required></label><label><span>邮箱地址</span><input type="email" id="cf-email" placeholder="name@example.com" required></label><label><span>想聊什么</span><textarea id="cf-message" rows="4" placeholder="简单介绍一下你的想法..."></textarea></label><button id="cf-send" class="btn primary" type="submit">发送消息 <span>↗</span></button></form></div></section>
  `;
...
function downloadResume() {
  ...
  const lines = [
    ...
    '',
    '工作经历',
    ...experience.map((item) => `${item.period} ${item.company} / ${item.role}\n${item.description}`),
    '',
    '能力',
    ...skills.map((skill) => `${skill.title}: ${skill.description}`),
    '',
    '作品',
